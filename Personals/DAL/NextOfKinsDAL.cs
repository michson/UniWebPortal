...
{
    public static class NextOfKinsDAL
    {
        public static String Insert(NextOfKin item)
        {
            ...
            else if (String.IsNullOrEmpty(item.RelTypeCode))
                return String.Format("RelTypeCode {0}", Messages.Warning);
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    context.NextOfKins.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.NextOfKins where (item.Code == Code) select item).FirstOrDefault();
                    context.NextOfKins.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(NextOfKin item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPNextOfKinsDelete]" })
                    {
        ...
        {
            List<NextOfKin> objs = new List<NextOfKin>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPNextOfKinsSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        NextOfKin items = new NextOfKin
                        {
                            Code = item.Code,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPNextOfKinsSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (NextOfKin item in items)
                        {
                            NextOfKin x = new NextOfKin
                            {
                                Code = item.Code,
                                AccountCode = item.AccountCode,
                                ScreenCode = item.ScreenCode,
