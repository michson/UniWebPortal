...
{
    public static class HobbiesDAL
    {
        public static String Insert(Hobby item)
        {
            ...
            else if (String.IsNullOrEmpty(item.HobbyCode))
                return String.Format("HobbyCode {0}", Messages.Warning);
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    context.Hobbies.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.Hobbies where (item.Code == Code) select item).FirstOrDefault();
                    context.Hobbies.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(Hobby item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPHobbiesDelete]" })
                    {
        ...
        {
            List<Hobby> objs = new List<Hobby>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPHobbiesSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        Hobby items = new Hobby
                        {
                            Code = item.Code,
                            AccountCode = item.AccountCode,
                            ScreenCode = item.ScreenCode,
                            HobbyCode = item.HobbyCode,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPHobbiesSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (Hobby item in items)
                        {
                            Hobby x = new Hobby
                            {
                                Code = item.Code,
