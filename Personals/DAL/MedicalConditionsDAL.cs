...
{
    public static class MedicalConditionsDAL
    {
        public static String Insert(MedicalCondition item)
        {
            ...
            else if (String.IsNullOrEmpty(item.DiseaseCode))
                return String.Format("DiseaseCode {0}", Messages.Warning);
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    context.MedicalConditions.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.MedicalConditions where (item.Code == Code) select item).FirstOrDefault();
                    context.MedicalConditions.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(MedicalCondition item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPMedicalConditionsDelete]" })
                    {
        ...
        {
            List<MedicalCondition> objs = new List<MedicalCondition>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPMedicalConditionsSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        MedicalCondition items = new MedicalCondition
                        {
                            Code = item.Code,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPMedicalConditionsSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (MedicalCondition item in items)
                        {
                            MedicalCondition x = new MedicalCondition
                            {
                                Code = item.Code,
                                AccountCode = item.AccountCode,
                                ScreenCode = item.ScreenCode,
