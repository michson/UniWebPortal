...
{
    public static class GuarantorsDAL
    {
        public static String Insert(Guarantor item)
        {
            ...
            else if (String.IsNullOrEmpty(item.Undertaking))
                return String.Format("Undertaking {0}", Messages.Warning);
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    context.Guarantors.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.Guarantors where (item.Code == Code) select item).FirstOrDefault();
                    context.Guarantors.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(Guarantor item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPGuarantorsDelete]" })
                    {
        ...
        {
            List<Guarantor> objs = new List<Guarantor>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPGuarantorsSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        Guarantor items = new Guarantor
                        {
                            Code = item.Code,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPGuarantorsSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (Guarantor item in items)
                        {
                            Guarantor x = new Guarantor
                            {
                                Code = item.Code,
                                AccountCode = item.AccountCode,
                                ScreenCode = item.ScreenCode,
