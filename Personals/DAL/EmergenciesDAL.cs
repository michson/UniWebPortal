...
{
    public static class EmergenciesDAL
    {
        public static String Insert(Emergency item)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    context.Emergencies.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.Emergencies where (item.Code == Code) select item).FirstOrDefault();
                    context.Emergencies.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(Emergency item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPEmergenciesDelete]" })
                    {
        ...
        {
            List<Emergency> objs = new List<Emergency>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPEmergenciesSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        Emergency items = new Emergency
                        {
                            Code = item.Code,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPEmergenciesSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (Emergency item in items)
                        {
