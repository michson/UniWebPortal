...
namespace Personals.DAL
{
    public static class OthersDAL
    {
        public static String Insert(Others item)
        {
            ...
            else if (String.IsNullOrEmpty(item.OthersCode))
                return String.Format("OthersCode {0}", Messages.Warning);
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.Others where (item.Code == Code) select item).FirstOrDefault();
                    context.Others.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(Others item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPOthersDelete]" })
                    {
        ...
        {
            List<Others> objs = new List<Others>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPOthersSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        Others items = new Others
                        {
                            Code = item.Code,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPOthersSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (Others item in items)
                        {
                            Others x = new Others
                            {
                                Code = item.Code,
