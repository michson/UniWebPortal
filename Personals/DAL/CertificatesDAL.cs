...
{
    public static class CertificatesDAL
    {
        public static String Insert(Certificate item)
        {
            ...
            else if (String.IsNullOrEmpty(item.YearObtained))
                return String.Format("YearObtained {0}", Messages.Warning);
            else if (String.IsNullOrEmpty(item.Title))
                return String.Format("Title {0}", Messages.Warning);
            else if (String.IsNullOrEmpty(item.Certificate1))
                return String.Format("Certificate1 {0}", Messages.Warning);
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    context.Certificates.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int32 Code)
        {
            ...
            using (PersonalEntities context = new PersonalEntities())
            {
                try
                {
                    var del = (from item in context.Certificates where (item.Code == Code) select item).FirstOrDefault();
                    context.Certificates.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(Certificate item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[Personals].[SPCertificatesDelete]" })
                    {
        ...
        {
            List<Certificate> objs = new List<Certificate>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(AccountCode) & !String.IsNullOrEmpty(ScreenCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var item = context.SPCertificatesSelect(Code, AccountCode, ScreenCode, Deleted).FirstOrDefault();
                        Certificate items = new Certificate
                        {
                            Code = item.Code,
                            AccountCode = item.AccountCode,
                            ScreenCode = item.ScreenCode,
                            AwardingBody = item.AwardingBody,
                            YearObtained = item.YearObtained,
                            Title = item.Title,
                ...
                else
                {
                    using (PersonalEntities context = new PersonalEntities())
                    {
                        var items = context.SPCertificatesSelect(Code, AccountCode, ScreenCode, Deleted);
                        foreach (Certificate item in items)
                        {
