...
namespace SetUp.DAL
{
    public static class CourseRegulationsDAL
    {
        public static String Insert(CourseRegulation item)
        {
            ...
            else if (String.IsNullOrEmpty(item.CreditLowerBound.ToString()))
                return String.Format("CreditLowerBound {0}", Messages.Warning);
            else if (String.IsNullOrEmpty(item.CreditUpperBound.ToString()))
                return String.Format("CreditUpperBound {0}", Messages.Warning);
            else if (String.IsNullOrEmpty(item.ModeOfStudyCode))
                return String.Format("ModeOfStudyCode {0}", Messages.Warning);
            ...
            using (SetUpEntities context = new SetUpEntities())
            {
                try
                {
                    context.CourseRegulations.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int64 Code)
        {
            ...
            using (SetUpEntities context = new SetUpEntities())
            {
                try
                {
                    var del = (from item in context.CourseRegulations where (item.Code == Code) select item).FirstOrDefault();
                    context.CourseRegulations.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(CourseRegulation item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[SetUp].[SPCourseRegulationDelete]" })
                    {
        ...
        {
            List<CourseRegulation> objs = new List<CourseRegulation>();
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(UniversityCode) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (SetUpEntities context = new SetUpEntities())
                    {
                        var item = context.SPCourseRegulationSelect(Code, UniversityCode, Deleted).FirstOrDefault();
                        CourseRegulation items = new CourseRegulation
                        {
                            Code = item.Code,
                            ...
                            CreditLowerBound = item.CreditLowerBound,
                            CreditUpperBound = item.CreditUpperBound,
                            ModeOfStudyCode = item.ModeOfStudyCode,
                ...
                else
                {
                    using (SetUpEntities context = new SetUpEntities())
                    {
                        var items = context.SPCourseRegulationSelect(Code, UniversityCode, Deleted);
                        foreach (CourseRegulation item in items)
                        {
                            CourseRegulation x = new CourseRegulation
                            {
                                Code = item.Code,
                                UniversityCode = item.UniversityCode,
                                FacultyCode = item.FacultyCode,
            ...
            {
                return new List<CourseRegulation>();
            }
        }
    }
}
