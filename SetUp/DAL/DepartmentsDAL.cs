...
namespace SetUp.DAL
{
    public static class DepartmentsDAL
    {
        public static String Insert(Department item)
        {
            ...
            using (SetUpEntities context = new SetUpEntities())
            {
                try
                {
                    context.Departments.AddObject(item);
                    context.SaveChanges();
        ...
        public static String DeletePermanently(Int64 Code)
        {
            ...
            using (SetUpEntities context = new SetUpEntities())
            {
                try
                {
                    var del = (from item in context.Departments where (item.Code == Code) select item).FirstOrDefault();
                    context.Departments.DeleteObject(del);
                    context.SaveChanges();
        ...
        public static String Delete(Department item)
        {
            ...
            try
            {
                ...
                {
                    using (SqlCommand cmd = new SqlCommand { Connection = con, CommandType = CommandType.StoredProcedure, CommandText = "[SetUp].[SPDepartmentsDelete]" })
                    {
        ...
        public static List<Department> Retrieve(Int64 Code, String UniversityCode, Int64 FacultyCode, Boolean Deleted)
        {
            ...
            try
            {
                if (!String.IsNullOrEmpty(Code.ToString()) & !String.IsNullOrEmpty(UniversityCode) & !String.IsNullOrEmpty(FacultyCode.ToString()) & !String.IsNullOrEmpty(Deleted.ToString()))
                {
                    using (SetUpEntities context = new SetUpEntities())
                    {
                        var item = context.SPDepartmentsSelect(Code, UniversityCode, FacultyCode, Deleted).FirstOrDefault();
                ...
                else
                {
                    using (SetUpEntities context = new SetUpEntities())
                    {
                        var items = context.SPDepartmentsSelect(Code, UniversityCode, FacultyCode, Deleted);
